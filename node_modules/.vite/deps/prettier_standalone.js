...
var ur = (e, t, r, n) => {
  if (t && typeof t == "object" || typeof t == "function") for (let o of pu(t)) !mu.call(e, o) && o !== r && $e(e, o, { get: () => t[o], enumerable: !(n = Fu(t, o)) || n.enumerable });
  return e;
};
...
var ir = Me((gt) => {
  ...
  or.prototype = { diff: function(t, r) {
    ...
    if (d[0].newPos + 1 >= s && f + 1 >= D) return i([{ value: this.join(r), count: r.length }]);
    function p() {
      for (var F = -1 * a; F <= a; F += 2) {
        ...
        if (!h || B && E.newPos < C.newPos ? (m = Au(C), u.pushComponent(m.components, void 0, true)) : (m = E, m.newPos++, u.pushComponent(m.components, true, void 0)), g = u.extractCommon(m, r, t, F), m.newPos + 1 >= s && g + 1 >= D) return i(yu(u, m.components, r, t, u.useLongestToken));
        d[F] = m;
      }
      a++;
    }
...
function Ar(e) {
  ...
  function n(u, i, s) {
    ...
    if (u.type === x) return `fill([${u.parts.map((c) => n(c)).join(", ")}])`;
...
var Or = (e) => {
  ...
  if (e.type !== x) throw new Error(`Expect doc to be 'array' or '${x}'.`);
  return e.parts;
};
function Ne(e, t) {
...
function Qe(e, t, r, n, o, u) {
  ...
  for (; r >= 0; ) {
    ...
    switch (U(c)) {
      ...
      case x: {
        let d = Or(c);
        for (let f = d.length - 1; f >= 0; f--) s.push({ mode: a, doc: d[f] });
        break;
      }
      case S:
      case T:
...
`, o) : l;
      ...
      case j:
        for (let F = l.length - 1; F >= 0; F--) i.push({ ind: f, mode: p, doc: l[F] });
        break;
      case W:
...
var bt = class {
  ...
  hasAncestor(t) {
    for (let r of ce(this, pe, et).call(this)) if (t(r)) return true;
    return false;
  }
};
...
function Ur(e, t) {
  ...
  for (let [p, l] of f.entries()) {
    ...
    let $;
    if (i ? $ = [l] : (F.enclosingNode = E, F.precedingNode = m, F.followingNode = C, $ = [F, g, h, B, Z]), Hu(g, h, f, p)) F.placement = "ownLine", a(...$) || (C ? te(C, F) : m ? re(m, F) : E ? X(E, F) : X(B, F));
    else if (qu(g, h, f, p)) F.placement = "endOfLine", c(...$) || (m ? re(m, F) : C ? te(C, F) : E ? X(E, F) : X(B, F));
    else if (F.placement = "remaining", !d(...$)) if (m && C) {
      let Q = n.length;
...
function Mr(e, t) {
  ...
  for (let a of [n, o]) a.comments && a.comments.length > 1 && a.comments.sort((c, d) => t.locStart(c) - t.locStart(d));
  e.length = 0;
}
function Pt(e, t, r) {
...
function ut({ plugins: e = [], showDeprecated: t = false } = {}) {
  ...
  for (let o of ro(Object.assign({}, ...e.map(({ options: u }) => u), Jr))) !t && o.deprecated || (Array.isArray(o.choices) && (t || (o.choices = o.choices.filter((u) => !u.deprecated)), o.name === "parser" && (o.choices = [...o.choices, ...to(o.choices, r, e)])), o.pluginDefaults = Object.fromEntries(e.filter((u) => {
    var i;
...
function so(e, t) {
  ...
  for (let o of io) o in t && (n[o] = Do(t[o], r, w.prototype[o].length));
  return n;
}
var w = class {
  static create(t) {
  ...
  _applyNormalization(t, r) {
    ...
    for (let i of o) {
      ...
      let a = ({ from: p, to: l }) => {
        ...
        if (F !== false) if (F === true) this._hasDeprecationWarned(i) || this._utils.logger.warn(this._deprecatedHandler(i, l, this._utils));
      ...
      for (let { from: p, to: l } of f.redirect) c({ value: p, redirectTo: l });
    }
    for (let i of u) {
      let s = t[i];
...
function fo(e, { isCLI: t, FlagSchema: r }) {
  ...
  for (let o of e) n.push(Fo(o, { isCLI: t, optionInfos: e, FlagSchema: r })), o.alias && t && n.push(st.create({ name: o.alias, sourceName: o.name }));
  return n;
}
...
async function mo(e, t = {}) {
  ...
  if (!r.parser) if (r.filepath) {
    if (r.parser = Zr(r, { physicalFile: r.filepath }), !r.parser) throw new Ie(`No parser could be inferred for file "${r.filepath}".`);
...
function Un(e, t, r) {
  ...
  if (No(t)) {
    let p = So(a, c);
    d = p, f = p;
  } else ({ startNode: d, endNode: f } = To(a, c, t));
...
async function Jn(e, t, r = 0) {
  ...
  if (t.cursorOffset >= 0) {
    let D, a, c, d, f;
    if (t.cursorNode && i.cursorNodeText ? (D = t.locStart(t.cursorNode), a = o.slice(D, t.locEnd(t.cursorNode)), c = t.cursorOffset - D, d = i.cursorNodeStart, f = i.cursorNodeText) : (D = 0, a = o, c = t.cursorOffset, d = 0, f = i.formatted), a === f) return { formatted: i.formatted, cursorOffset: d + c, comments: s };
    let p = a.split("");
    p.splice(c, 0, zn);
...
async function Zt(e, t) {
  ...
  if (o.rangeStart >= o.rangeEnd && n !== "" || o.requirePragma && !await Kn(n, o)) return { formatted: e, cursorOffset: t.cursorOffset, comments: [] };
  let u;
